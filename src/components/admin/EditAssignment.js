...
const EditAssignment = ({ isOpen, onRequestClose, id }) => {
  const [assignment, setAssignment] = useState("");
  const [totalMark, setTotalMark] = useState("");
  const [videoId, setVideoId] = useState("");
  const [show, setShow] = useState(true);
  const { data: getAssignment } = useGetAssignmentQuery(id);
  ...
  useEffect(() => {
    setAssignment(getAssignment?.title);
    setTotalMark(getAssignment?.totalMark);
    setVideoId(getAssignment?.video_id);
  }, [getAssignment]);
        ...
        <div className="grid grid-cols-2 gap-4 text-black w-[40vw] max-h-[80vh]">
          ...
          <div className="col-span-2">
            ...
            {show ? (
              ...
              >
                {getAssignment && getAssignment.video_title}
              </div>
            ) : (
              <select
                className="w-full px-4 py-2 border rounded-lg bg-slate-300"
